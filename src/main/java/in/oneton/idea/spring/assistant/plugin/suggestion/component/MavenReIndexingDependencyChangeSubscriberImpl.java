...
public class MavenReIndexingDependencyChangeSubscriberImpl
    ...
    private static final Logger log = Logger.getInstance(MavenReIndexingDependencyChangeSubscriberImpl.class);
        ...
        try {
            ...
            connection.subscribe(MavenImportListener.TOPIC, (importedProjects, newModules) -> {
                boolean proceed = importedProjects.stream().anyMatch(
                        project -> this.project.getName().equals(project.getName())
                                && project.getDirectory().equals(this.project.getBasePath()));
